    ...
    return (
        <div className="flex justify-center">
            <div className="w-12 m-8">
                <Link to={`/list?category=${category}&page=${currentPage - 1}`} className={`fixed h-12 w-12 top-1/2 ${currentPage == 1 ? "hidden" : ""}`}>
                    <img src={leftIcon} />
                </Link>
            </div>
            ...
            <div className="w-12 m-8">
                <Link to={`/list?category=${category}&page=${currentPage + 1}`} className={`fixed h-12 w-12 top-1/2 ${currentPage >= searchResults.totalPages ? "hidden" : ""}`}>
                    <img src={rightIcon} />
                </Link> 
            </div>
        </div>
